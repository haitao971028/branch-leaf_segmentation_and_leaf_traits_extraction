...
def parse_args():
    ...
    parser.add_argument('--epoch',  default=800, type=int, help='Epoch to run [default: 251]')
...
def main(args):
    ...
    weights = torch.Tensor(np.array([0.9,0.1])).cuda()
    ...
    try:
        checkpoint = torch.load(str(experiment_dir) + '/checkpoints/best_model1111111.pth')
    ...
    for epoch in range(start_epoch,args.epoch):
        ...
        for i, data in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), smoothing=0.9):
            ...
            statistics = target.data.numpy()
            statistics = statistics.reshape(-1,1)
            weight1 = np.sum(statistics)
            weight2 = len(statistics)-weight1
            weight = torch.Tensor(np.array([weight1,weight2])).cuda()
    ...
    np.savetxt("train_loss.txt",train_loss,fmt="%f")
    np.savetxt("train_acc.txt",train_acc,fmt="%f")
